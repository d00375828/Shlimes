...
void configureAudioQualityUI(ApplicationData& app_data) {
    ...
    if (quality == "low") {
        app_data.getAudioTrack().setSamplesPerSecond(8000);
        app_data.getWAVFile().setBitsPerSample(8);
    } else if (quality == "mid") {
        app_data.getAudioTrack().setSamplesPerSecond(16000);
        app_data.getWAVFile().setBitsPerSample(16);
    } else if (quality == "high") {
        app_data.getAudioTrack().setSamplesPerSecond(32000);
        app_data.getWAVFile().setBitsPerSample(16);
    } else if (quality == "CD") {
        app_data.getAudioTrack().setSamplesPerSecond(44100);
        app_data.getWAVFile().setBitsPerSample(16);
    } else {
    ...
    app_data.getWAVFile().setSamplesPerSecond(app_data.getAudioTrack().getSamplesPerSecond());
}
...
int register_instrument_designer_commands(ApplicationData& app_data) {
    ...
    app_data.addAction(ActionFunctionData("configure-audio-quality", 
                                         configureAudioQualityUI, 
                                         "Configure the sample rate and bit depth."));
    
    app_data.addAction(ActionFunctionData("configure-duration", 
                                         configureTrackDurationUI, 
